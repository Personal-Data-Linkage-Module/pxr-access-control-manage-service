...
/**
 * If array every value is not empty string
 * @param validationOptions
 */
export function IsStringEmptyInArray (validationOptions?: ValidationOptions) {
    return function (object: Object, propertyName: string) {
        registerDecorator({
            ...
            validator: {
                validate (value: any, args: ValidationArguments) {
                    return Array.isArray(value) && value.every(elem => typeof elem === 'string' && elem !== '');
                }
            }
        });
    };
