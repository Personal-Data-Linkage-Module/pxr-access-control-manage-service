...
const session = encodeURIComponent(JSON.stringify({
    ...
    auth: { member: { add: true, update: true, delete: true }, join: { create: true, update: true } },
...
describe('Access-Control Manage Service.Create Join API Key', () => {
    ...
    describe('参加時APIトークン生成指示 POST: ' + baseURI, () => {
        beforeAll(async () => {
        ...
        test('正常系: 参加時APIトークン生成指示(PXR-Root)', async () => {
            const response = await supertest(expressApp)
                .post(baseURI)
                .set({
                    accept: 'application/json',
        ...
        test('正常系: 参加時APIトークン生成指示(RoleEntity)', async () => {
            const response = await supertest(expressApp)
                .post(baseURI)
                .set({
                    accept: 'application/json',
        ...
        test('異常系: 参加時APIトークン生成指示(ブロックカタログではないNS)', async () => {
        ...
        test('異常系: 参加時APIトークン生成指示(ブロックカタログとしては期待しない形式)', async () => {
    ...
    describe('参加時APIトークン生成指示 POST: ' + baseURI, () => {
        ...
        test('正常系: 参加時APIトークン生成指示(空)', async () => {
            const response = await supertest(expressApp)
                .post(baseURI)
                .set({
                    accept: 'application/json',
