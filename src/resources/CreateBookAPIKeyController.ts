...
export default class CreateBookAPIKeyController {
    @Post('/book')
    ...
    async post (
        ...
        for (const item of dto) {
            // 宛先と権限を評価する
            await CreateAPIKeyService.checkPermission(item, operator, 'book');

            // ログインオペレーターがData-Traderであれば、例外をスローする
            await CreateAPIKeyService.checkBookCreation(operator);
