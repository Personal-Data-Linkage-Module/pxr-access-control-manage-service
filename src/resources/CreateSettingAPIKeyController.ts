...
export default class CreateSettingAPIKeyController {
    @Post('/setting')
    ...
    async post (
        ...
        for (const item of dto) {
            // 宛先と権限を評価する
            await CreateAPIKeyService.checkPermission(item, operator, 'setting');

            // 操作可能なネームスペースか確認する
            // await CreateAPIKeyService.checkNs(item.caller.requestBody, 'manageGlobalSetting');
