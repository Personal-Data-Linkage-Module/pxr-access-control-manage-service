...
export default class CreateOperatorAPIKeyController {
    @Post('/operator')
    ...
    async post (
        ...
        for (const item of dto) {
            // 宛先と権限を評価する
            await CreateAPIKeyService.checkPermission(item, operator, 'member');
            if (item.caller.requestBody) {
                await CreateAPIKeyService.checkOperatorAuth(item.caller.requestBody, operator);
            }
